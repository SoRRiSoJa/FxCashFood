 ...
 * @author joao
 */
public class BoxSangrarCaixaFXMLController implements Initializable {
    ...
    @FXML
    private JFXButton btnSangrarCaixa;
    @FXML
    private JFXButton btnCancelar;
    @FXML
    private JFXTextField txtSaldoAnteriro;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
        txtSaldoAnteriro.setText(CaixaController.getInstance().getSaldoFinal().toString());
        loadDateTime();
        loadUser();
    }
    ...
    private void onAbrir(ActionEvent event) {
        getData();
        if (validateField()) {
            CaixaController.getInstance().sangrarCaixa(valor);
    ...
    private boolean validateField() {
        ...
        if (valor.compareTo(CaixaController.getInstance().getSaldoFinal()) > 0) {
            erros += "O valor da sangria não pode ser superior ao saldo em caixa.";
            flag = false;
        }
        return flag;
    }
